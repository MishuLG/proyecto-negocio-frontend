...
export default function Dashboard() {
    ...
    return (
        <div className={isDark ? 'dark' : ''}>
            <div className="p-10 bg-slate-50 dark:bg-slate-900 min-h-screen font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">

                <div className="flex justify-between items-center mb-10 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white">⚙️ Panel de Control</h1>
                    <div className="flex gap-4">
                        <button onClick={() => setIsDark(!isDark)} className="bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-white px-4 py-3 rounded-xl font-bold transition-all">
                        ...
                        <button onClick={() => { localStorage.clear(); navigate('/'); }} className="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50 px-6 py-3 rounded-xl font-bold transition-all">Salir</button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <input type="number" value={tasaCOP} onChange={e => setTasaCOP(e.target.value)} className="flex-1 px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500" placeholder="Ej: 3900" />
                                <button onClick={() => actualizarTasa('COP', tasaCOP)} className="bg-slate-900 dark:bg-blue-600 text-white px-6 py-3 rounded-xl font-bold">Guardar</button>
                            </div>
                            <div className="flex items-center gap-4">
                                <span className="font-bold text-slate-600 dark:text-slate-400 w-auto whitespace-nowrap">BS (Tasa Frontera):</span>
                                <input type="number" step="0.1" value={tasaBS} onChange={e => setTasaBS(e.target.value)} className="flex-1 px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500" placeholder="Ej: 5 (5000 COP = 1000 BS)" />
                                <button onClick={() => actualizarTasa('BS', tasaBS)} className="bg-slate-900 dark:bg-blue-600 text-white px-6 py-3 rounded-xl font-bold">Guardar</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
                        ...
                        <form onSubmit={agregarProducto} className="space-y-4">
                            ...
                            <div className="flex gap-4">
                                <input type="number" step="100" placeholder="Precio en COP" required value={nuevoProd.precio_cop} onChange={e => setNuevoProd({ ...nuevoProd, precio_cop: e.target.value })} className="w-1/2 px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500" />
                                <select value={nuevoProd.categoria_id} onChange={e => setNuevoProd({ ...nuevoProd, categoria_id: e.target.value })} className="w-1/2 px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white">
                                    <option value="1">Noche (Hamburguesas/Perros)</option>
                                    <option value="2">Mañana (Pasteles/Desayunos)</option>
                                </select>
                            </div>
                            <button type="submit" className="w-full bg-green-500 hover:bg-green-600 text-white py-4 rounded-xl font-bold text-lg shadow-md mt-2 transition-colors">Crear Producto</button>
                        </form>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8 transition-colors">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setTurnoCatalogo('Todos')} className={`px-4 py-2 rounded-xl font-bold text-sm transition-all ${turnoCatalogo === 'Todos' ? 'bg-slate-800 dark:bg-slate-600 text-white shadow-md' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300'}`}>📦 Todos</button>
                            <button onClick={() => setTurnoCatalogo('Mañana')} className={`px-4 py-2 rounded-xl font-bold text-sm transition-all ${turnoCatalogo === 'Mañana' ? 'bg-amber-500 text-white shadow-md' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400'}`}>☀️ Mañana</button>
                            <button onClick={() => setTurnoCatalogo('Noche')} className={`px-4 py-2 rounded-xl font-bold text-sm transition-all ${turnoCatalogo === 'Noche' ? 'bg-indigo-600 text-white shadow-md' : 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400'}`}>🌙 Noche</button>
                        </div>
                    </div>
                            ...
                            <tbody>
                                {productosFiltrados.length === 0 ? (
                                    ...
                                    productosFiltrados.map(prod => (
                                        <tr key={prod.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            {editandoId === prod.id ? (
                                                <>
                                                    <td className="p-2"><input type="text" value={datosEdicion.nombre} onChange={e => setDatosEdicion({ ...datosEdicion, nombre: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white" /></td>
                                                    <td className="p-2"><input type="number" step="100" value={datosEdicion.precio_cop} onChange={e => setDatosEdicion({ ...datosEdicion, precio_cop: e.target.value })} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white" /></td>
                                                    <td className="p-2">
                                                    ...
                                                    <td className="p-2 flex gap-2 justify-center">
                                                        ...
                                                        <button onClick={() => setEditandoId(null)} className="bg-slate-300 dark:bg-slate-600 text-slate-800 dark:text-white px-3 py-2 rounded-lg font-bold text-sm">Cancelar</button>
                                                    </td>
                                                </>
                                            ) : (
                                                <>
                                                    ...
                                                    <td className="p-4 flex gap-2 justify-center">
                                                        <button onClick={() => iniciarEdicion(prod)} className="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-4 py-2 rounded-lg font-bold text-sm">✏️</button>
                                                        <button onClick={() => eliminarProducto(prod.id, prod.producto || prod.nombre)} className="bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400 px-4 py-2 rounded-lg font-bold text-sm">🗑️</button>
                                                    </td>
                                                </>
                                            )}
                                        </tr>
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8 transition-colors">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="font-bold text-slate-500 dark:text-slate-400">Filtrar por Fecha:</span>
                            <input
                                type="date"
                                value={fechaFiltro}
                                onChange={(e) => setFechaFiltro(e.target.value)}
                                className="px-4 py-2 rounded-xl border-2 border-slate-200 dark:border-slate-600 outline-none font-bold text-slate-700 dark:text-white bg-slate-50 dark:bg-slate-700 focus:border-blue-500 transition-colors cursor-pointer"
                            />
                        </div>
                    </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {ventasFiltro.length === 0 ? (
                                <tr><td colSpan="7" className="p-4 text-center text-slate-500 dark:text-slate-400">No hay ventas registradas en esta fecha ({fechaFiltro})</td></tr>
                            ) : (
                                ventasFiltro.map(venta => {
                                    ...
                                    return (
                                        <tr key={venta.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            ...
                                            <td className="p-4">
                                                {esPagado ? (
                                                    <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-3 py-1 rounded-full font-bold text-xs">✅ Pagado</span>
                                                ) : (
                                                    <span className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 px-3 py-1 rounded-full font-bold text-xs">⏳ Pendiente</span>
                                                )}
                                            </td>
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 border-l-4 border-l-amber-500 transition-colors">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold text-slate-800 dark:text-white">⚠️ Control de Fiados (Cuentas por Cobrar)</h2>
                        <span className="font-black text-red-500 dark:text-red-400 text-lg bg-red-50 dark:bg-red-900/30 px-4 py-2 rounded-lg">
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {deudores.length === 0 ? (
                                ...
                                deudores.map(deudor => (
                                    <tr key={deudor.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                                        ...
                                        <td className="p-4 text-right">
                                            <button onClick={() => pagarDeuda(deudor.id, deudor.nombre, deudor.deuda_total)} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-bold text-sm transition-colors">
                                                💰 Abonar / Saldar
                                            </button>
                                        </td>
                                    </tr>
